...
export class PerformanceMonitor {
   ...
   */
  end(name: string, logResult = false): number | undefined {
    if (!this.enabled) {
      return undefined;
    }
   ...
   */
  async measure<T>(name: string, fn: () => Promise<T>, logResult = false): Promise<T> {
    this.start(name);
    try {
   ...
   * @returns The result of the function
   */
  measureSync<T>(name: string, fn: () => T, logResult = false): T {
    this.start(name);
    try {
      const result = fn();
      this.end(name, logResult);

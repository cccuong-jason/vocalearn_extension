...
export interface TranslationPopupData {
...
export class TranslationPopup {
  ...
  private word: string = '';
  private data: TranslationPopupData = {};
  ...
  async show(x: number, y: number, word: string, data: TranslationPopupData): Promise<void> {
  ...
  private async saveWord(): Promise<void> {
    ...
    const saveBtn = this.popup.querySelector('.vocalearn-save') as HTMLButtonElement;

...
const Popup = () => {
    ...
    const translateText = useCallback((text) => {
        ...
        setTimeout(() => {
            const translatedText = `[${languages[targetLanguage].name} translation of: "${text}"]`;
    ...
    const handleUpdateWord = useCallback((updatedWord) => {
        setVocabularyWords(prev => {
            const updatedWords = prev.map(word => word.id === updatedWord.id ? updatedWord : word);
            // Save to storage
            if (typeof chrome !== 'undefined' && chrome.storage) {
                chrome.storage.local.set({ vocabulary: updatedWords });
            }
        ...
        switch (activeTab) {
            case 'translate':
                return (React.createElement(React.Fragment, null,
                    React.createElement(LanguageSelector, { sourceLanguage: languages[sourceLanguage], targetLanguage: languages[targetLanguage], onSwapLanguages: swapLanguages }),
                    React.createElement(TranslationPanel, { isSource: true, languageName: languages[sourceLanguage].name, text: sourceText, charCount: sourceText.length, maxChars: 2300, onChange: handleSourceTextChange, onClear: clearText, onSpeak: () => speakText(sourceText, sourceLanguage) }),
                    React.createElement("div", { className: "divider" }),
                    React.createElement(TranslationPanel, { isSource: false, languageName: languages[targetLanguage].name, text: translatedText, isTranslating: isTranslating, charCount: translatedText.length, maxChars: 2300, onCopy: copyTranslation, onSpeak: () => speakText(translatedText, targetLanguage) })));
            case 'vocabulary':
                return React.createElement(VocabularyList, { vocabularyWords: vocabularyWords, onDeleteWord: handleDeleteWord, onUpdateWord: handleUpdateWord });
            case 'settings':
                return React.createElement(SettingsPanel, null);
            default:
                return React.createElement("div", { className: "coming-soon" }, "This feature is coming soon!");
        }
    };
    return (React.createElement("div", { className: "popup" },
        React.createElement("header", { className: "header" },
            React.createElement("h1", { className: "title" }, "Vocalearn"),
            React.createElement("button", { className: "notification-btn" },
                React.createElement("i", { className: "fas fa-bell" }))),
        React.createElement("div", { className: "content" }, renderActiveContent()),
        React.createElement(NavigationBar, { items: navigationItems, activeItem: activeTab, onItemClick: handleNavigation })));
};
